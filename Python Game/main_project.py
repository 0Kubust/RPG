...
monster_1 = character(600, 300, "monster", 6, 4, 1, 1, 4, 0, 1)
monster_2 = character(720, 300, "monster", 6, 4, 1, 1, 4, 0, 1)
all_list = []
monster_list = []
...
while run:
    ...
    if lv_points == 0 and knight.lv == 2: 
        ...
        if arrow.collidepoint(pos):
            if clicked == True and pygame.time.get_ticks() - time_keeper > 3000:
                victory_img = pygame.Surface((0,0))
                click_kepper = True
        
        if click_kepper == True:
            screen.blit(mapa,(0,0))  
            #Options   
            imag_rect = pygame.Rect(75 , 480 ,70,55) 
            imag_rect1 = pygame.Rect(250 , 480 ,75,55) 
            imag_rect2 = pygame.Rect(455 , 480 ,75,60) 
            imag_rect3 = pygame.Rect(660 , 480 ,60,55) 
    
            if imag_rect.collidepoint(pos):
                pygame.mouse.set_visible(False), screen.blit(sword_img, pos) , screen.blit(mapaeq,(0,0))
            if imag_rect1.collidepoint(pos):
                screen.blit(sword_img, pos)
                screen.blit(mapacharacter, (0,0))
            if imag_rect2.collidepoint(pos):
                screen.blit(sword_img, pos)
                screen.blit(mapalore, (0,0))
            if imag_rect3.collidepoint(pos):
                screen.blit(sword_img, pos)
                screen.blit(mapaoptions, (0,0))
